...
public class MessageDB {
    ...
    public void add(String userId, ChatMessage chatMessage) {
        ...
        int isComing = chatMessage.getIsComing() == 2 ? 1 : 0;
        int readed = chatMessage.isReaded() ? 1 : 0;
        mDb.execSQL("insert into _" + userId + " (" + COL_USER_ID + "," + COL_ICON + "," + COL_IS_COMING + ","
                + COL_MESSAGE + "," + COL_NICKNAME + "," + COL_READED + "," + COL_DATE + ") values(?,?,?,?,?,?,?)",
                new Object[] { chatMessage.getUserId(), chatMessage.getIcon(), isComing, chatMessage.getMessage(),
                        chatMessage.getNickname(), readed, chatMessage.getDateStr() });
    }
    ...
    public List<ChatMessage> find(String userId, int currentPage, int pageSize) {
        ...
        // ȡ����10��
        String sql = "select * from _" + userId + " order by _id  desc limit  " + start + " , " + end;
        ...
        while (c.moveToNext()) {
            ...
            chatMessage = new ChatMessage(message, isComingVal, _userId, icon, nickname, readedVal == 1, dateStr);
    ...
    private int getUnreadedMsgsCountByUserId(String userId) {
        createTable(userId);
        String sql =
                "select count(*) as count from _" + userId + " where " + COL_IS_COMING + " = 1 and " + COL_READED
                        + " = 0";
        Cursor c = mDb.rawQuery(sql, null);
        int count = 0;
        if (c.moveToNext())
    ...
    public void updateReaded(String userId) {
        createTable(userId);
        mDb.execSQL("update  _" + userId + " set " + COL_READED + " = 1 where " + COL_READED + " = 0 ", new Object[] {});
    }

    private void createTable(String userId) {
        mDb.execSQL("CREATE table IF NOT EXISTS _" + userId + " (_id INTEGER PRIMARY KEY AUTOINCREMENT, " //
                + COL_USER_ID + " TEXT, " //
                + COL_ICON + " integer, "//
